...
def week_2_ingestion():
    ...
    def load(date, bucket):
        ...
        upload_blob(bucket, f'{WORK_DIR}yellow/{year}-{month}.csv', f'yellow/{year}-{month}.csv')
        upload_blob(bucket, f'{WORK_DIR}green/{year}-{month}.csv', f'green/{year}-{month}.csv')

    @task(
        trigger_rule = 'all_success'
    )
    def big_query_insert(date, bucket):
        ...
        for taxi in (['yellow', 'green']):
            green_query = f"""
                                ...
                                uris = ['gs://{bucket}/{taxi}/{year}-{month}.csv'],
                                format = 'csv'
                                );
    ...
    inserter = big_query_insert(date, checker)
    cleaner = clean()

    ingester >> checker >> loader >> inserter >> cleaner
